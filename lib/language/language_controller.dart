...
class LanguageController extends GetxController {
  ...
  static const String languagesCacheKey = 'languagesCache';

  Future<void> fetchLanguages() async {
    _isLoading.value = true;
    try {
      final box = GetStorage();
      // Try to load languages from cache
      final cachedLanguages = box.read(languagesCacheKey);
      if (cachedLanguages != null) {
        final List<dynamic> cachedData = json.decode(cachedLanguages);
        languages = cachedData.map((json) => Language.fromJson(json)).toList();
        ...
        // Cache languages
        final languagesJson = json
            .encode(languages.map((language) => language.toJson()).toList());
        box.write(languagesCacheKey, languagesJson);
        _isLoading.value = false;
      }
    } catch (e) {
